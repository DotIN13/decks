 ...
 * costs a live document every time you look at it. So the browse modal photographs a board
 * once and shows the photograph afterwards, keyed `path@rev` so an edit misses the cache by
 ...
 * - the context panel never uses one, because those are the boards being rewritten;
...
try {
	 ...
	 */
	await page.waitForFunction(() => document.querySelectorAll(".all-boards .thumb-shot").length > 0, null, { timeout: 30000 });
	...
	/*
	 * Asserted on reopen rather than in place. While the modal is up a board that has been
	 * photographed swaps to its picture, but the *point* of the cache is the second look:
	 * closing and reopening is what used to rebuild every document.
	 */
	...
	const ink = await page.evaluate(async () => {
		const img = document.querySelector(".all-boards .thumb-shot");
		if (!img) return null;
		await img.decode();
	 ...
	 */
	const target = second.find((entry) => entry.kind === "photograph")?.path;
	...
	// --- 5. the context panel is never photographed ----------------------------------
	/*
	 * Those are the boards an agent is rewriting. A picture of one would be out of date
	 * before it landed, and re-taken on every revision — which is more work than the live
	 * document it replaced, for a worse answer.
	 */
	await page.keyboard.press("Escape");
