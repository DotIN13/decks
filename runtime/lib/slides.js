/**
 * A slide deck, drawn from reveal's markdown dialect.
 *
 ...
 * — transitions, fragments, nested stacks, auto-animate, PDF export — costs about a megabyte
 * and brings its own keyboard layer, its own fullscreen and its own CSS reset, each of which
 * would have to be contained so it did not fight the canvas. The three renderers a slide
 * actually needs (marked, KaTeX, mermaid) are already vendored here for boards.
 *
 * Keeping reveal's dialect rather than inventing one is the portability: a deck written for
 ...
 *
 * `Note:` at the start of a line, to the end of the slide — reveal's convention. Parsed and
 * kept rather than dropped, so that the day a presenter view exists the content is already
 * in the file; hidden by CSS until then.
 */
export function splitNote(slide) {
	const at = /^Note:[ \t]*/m.exec(slide);
 ...
 *
 * `board.js` calls this when it sees `data-slides`, hands it a way to render markdown — its
 * own, so a slide gets the same marked + KaTeX + mermaid a `[data-md]` component does — and
 * gets back a handle the app drives with the arrow keys.
 *
...
export function mountDeck(host, source, options) {
	const { meta, slides } = splitDeck(source);
	...
	const slideEl = (index, into) => {
		...
		el.className = "slide";
		const { body, note } = splitNote(slides[index] || "");
		into.appendChild(el);
