...
export function createCanvasMorph(options: { deckPath: () => string; stage: () => Element | null; camera: () => Camera; storage?: Storage }): CanvasMorph {
	...
	/**
	 * Run the grow (or the shrink) frame by frame, on the app's own curve.
	 *
	 * Not the stage's glide: that one moves the camera's centre linearly, which with a tenfold
	 * change of zoom swings the boards off the screen in the middle of the move. `morphFrame`
	 * keeps them on a straight line from the card to their place instead. Each frame is an
	 * ordinary camera set, the same thing a finger's pan does.
	 */
