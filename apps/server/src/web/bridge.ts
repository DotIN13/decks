...
export class WebBridge {
	...
	/** Type into a field, found by its label, placeholder or accessible name. */
	async fill(field: string, text: string): Promise<{ field: string }> {
		const page = await this.page();
		try {
			const target = await firstOf(field, [
			...
			this.note(`filled ${field}`, true);
			return { field };
		} catch (error) {
			this.note(`could not fill ${field}: ${(error as Error).message}`, false);
			throw error;
		}
	}

	/** Pick an option in a `<select>`, by its label or value. */
	async select(field: string, option: string): Promise<{ field: string; option: string }> {
		const page = await this.page();
		try {
			const target = await firstOf(field, [page.getByLabel(field), page.getByRole("combobox", { name: field })]);
			await target.selectOption({ label: option }, { timeout: 10_000 }).catch(() => target.selectOption(option, { timeout: 10_000 }));
			this.note(`chose ${option} for ${field}`, true);
			return { field, option };
		} catch (error) {
			this.note(`could not choose ${option} for ${field}: ${(error as Error).message}`, false);
			throw error;
		}
	}

	/** Click a button, link, checkbox or piece of text, by its name. */
	async click(what: string): Promise<{ clicked: string }> {
		const page = await this.page();
		try {
			const target = await firstOf(what, [
				page.getByRole("button", { name: what, exact: true }),
				page.getByRole("button", { name: what }),
				page.getByRole("link", { name: what }),
				page.getByRole("checkbox", { name: what }),
				page.getByRole("radio", { name: what }),
				page.getByRole("tab", { name: what }),
				page.getByRole("menuitem", { name: what }),
				page.getByLabel(what),
				page.getByText(what, { exact: true }),
				page.getByText(what),
			]);
			await target.click({ timeout: 10_000 });
			this.note(`clicked ${what}`, true);
			return { clicked: what };
		} catch (error) {
			this.note(`could not click ${what}: ${(error as Error).message}`, false);
			throw error;
		}
	}
	 ...
	 */
	async submit(what?: string, options?: { ask?: boolean }): Promise<{ submitted: string; allowed: boolean }> {
		const page = await this.page();
		const label = what ? `press "${what}"` : "press Enter to submit";
		if (options?.ask !== false) {
			...
			if (!allowed) {
				...
				return { submitted: what ?? "Enter", allowed: false };
			}
		}
		if (what) await this.click(what);
		else {
			await page.keyboard.press("Enter");
			this.note("pressed Enter to submit", true);
		}
		return { submitted: what ?? "Enter", allowed: true };
	}
...
/** The first locator that matches something on the page, or a sentence naming what did not. */
async function firstOf(name: string, candidates: Locator[]): Promise<Locator> {
	for (const candidate of candidates) {
		const one = candidate.first();
		if ((await one.count()) > 0) return one;
	}
